...
### Issue #4 — Notified for playlist adds, but not for ratings (stretch)

**How I reproduced it.** This bug is the *absence* of a function call, so I ran the real, unmodified `services/notification_service.py` against a minimal fake ORM (Flask/SQLAlchemy unavailable in this sandbox — see note at end; the fake only implements the `db.session.get/add/commit` and `.query().filter_by().first()` calls this file actually uses) and called both `add_to_playlist()` and `rate_song()` with equivalent "someone else interacts with my song" setups. `add_to_playlist` produced 1 `Notification`; `rate_song` produced 0 — reproduced.

**How I found the root cause.** Per the hint, I compared the two flows line by line instead of guessing. `add_to_playlist()`: (1) look up the song, adder, playlist, (2) mutate — append song to playlist, commit, (3) **notify** — call `create_notification(user_id=song.shared_by, ...)` if the adder isn't the sharer. `rate_song()`: (1) look up the song and rater, (2) mutate — create/update the `Rating`, commit, (3) *return*. Step 3 in `add_to_playlist` has no counterpart in `rate_song` at all. That's what made this "architectural" rather than a typo: there's no broken conditional to fix, just a whole step of the established pattern that was never written for this second flow.

**The root cause.** `notification_service.py` establishes a "mutate, then notify the original sharer" pattern for interactions with someone else's song, implemented in `add_to_playlist()`. `rate_song()` performs the mutate step (saving the `Rating`) but was never extended to call `create_notification()` afterward, so no `Notification` row is ever created when a friend rates your song, even though the notification list/read endpoints (`routes/users.py`) are fully generic and would display it correctly if it existed.

**My fix and side-effect check.** After `rate_song()` commits the rating, I added the same shape of call `add_to_playlist` uses: if `song.shared_by != user_id`, call `create_notification(user_id=song.shared_by, notification_type="song_rated", body=f"{rater.username} rated your song '{song.title}' {score}/5.")`. I verified three cases against the harness: (1) a friend rating your song now produces exactly one `song_rated` notification to the sharer; (2) a user rating their **own** shared song produces zero notifications (no self-notification, matching the `song.shared_by != added_by_user_id` guard already used in `add_to_playlist`); (3) re-rating an already-rated song (the `existing.score = score` branch) still runs cleanly and notifies again without raising — so the existing "update rating" behavior in `rate_song` is unaffected by the addition.

### Issue #5 — The last song in a playlist never shows up (stretch)

**How I reproduced it.** `get_playlist_songs()` builds `songs` as a correctly-ordered list (via `.order_by(asc(playlist_entries.c.position))`) and only its final return line is in question, so I reproduced the bug on a plain 5-item list standing in for that already-ordered result (Flask/SQLAlchemy unavailable in this sandbox — see note at end; the ordering logic itself was not in doubt, only the final slice). Running the actual return expression, `[song.to_dict() for song in songs[:-1]]`, against 5 songs returned 4 — `Track 5` was silently missing — reproduced. This matches `tests/test_playlists.py`'s own comment: `assert len(songs) == 5  # Bug causes this to return 4`.

**How I found the root cause.** `routes/playlists.py::get_songs()` calls `playlist_service.get_playlist_songs()` directly, so I read that function top to bottom. The query construction (join, filter by playlist, order by position) is correct and matches the docstring. The only place a song could get dropped is the return line: `return [song.to_dict() for song in songs[:-1]]`. The docstring even says "Note: This function returns all songs in the playlist" directly above code that provably doesn't — that contradiction between the comment and the code was what confirmed this was the exact cause rather than something upstream in the query.
